...
@pytest.fixture
def curation_referencing_uncached_asset(_isolated_curation_root, monkeypatch):
    """Curation whose ``peptide_attributions`` CSV is neither packaged nor cached,
    on top of ``_isolated_curation_root`` (#474) -- this fixture only owns the
    content specific to this test: the unreachable attribution path, and
    refusing the network fetch it would otherwise trigger.
    """
